...
const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed w-full z-50 glass-morphism px-8 py-4 flex justify-between items-center transition-all duration-300 ${isHighContrast ? "bg-black border-yellow-400" : ""}`}
    >
      <div className="flex items-center gap-6">
        <div
          ...
          onClick={() => navigate("/")}
        >
          <div className="rounded-lg text-white w-10">
            <img
              src="/logo.jpg"
          ...
          <span
            className={`text-2xl font-bold tracking-tight ${isHighContrast ? "text-white" : "text-medical-primary"}`}
          >
            MetQ.
          </span>
        </div>
      ...
      <div className="flex items-center gap-4">
        {user ? (
                ...
                <div className="absolute right-0 mt-3 w-56 bg-white rounded-3xl premium-shadow border border-gray-100 overflow-hidden z-50">
                  ...
                  <div className="p-2">
                    ...
                    >
                      <LayoutDashboard
                        size={16}
                        className="text-gray-400 group-hover:text-medical-primary"
                      />
          ...
          <Link
            to="/book"
            className="px-8 py-2.5 bg-black text-white rounded-full text-sm font-semibold hover:scale-105 transition-all premium-shadow active:scale-95"
          >
            Book a call
          </Link>
        )}
      ...
      {isMobileMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-white md:hidden flex flex-col shadow-lg border-t z-50">
          {[
            { name: "Home", path: "/" },
            { name: "About", path: "/about" },
            { name: "Technology", path: "/technology" },
            ...
            <Link
              key={link.path}
              to={link.path}
              className={`p-4 border-b border-gray-50 text-sm font-bold tracking-widest uppercase transition-colors ${isActive(link.path) ? "text-medical-primary bg-medical-soft" : "text-gray-600 hover:bg-gray-50 hover:text-medical-primary"
                }`}
          ...
          {!user && (
            <div className="p-4 bg-gray-50">
              <Link
                to="/book"
                className="w-full inline-block px-8 py-3 bg-black text-white rounded-full text-sm font-bold uppercase tracking-widest text-center hover:scale-[1.02] transition-all premium-shadow"
                onClick={() => setIsMobileMenuOpen(false)}
              >
                Book a call
              </Link>
