...
const MedicalPassport = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const url = patientId
                    ? `http://localhost:5000/api/medical/passport/${patientId}`
                    : 'http://localhost:5000/api/medical/passport';
                const res = await axios.get(url);

                if (res.data && res.data.records) {
                    res.data.records = res.data.records.map(r => {
                        let parsedLab = r.lab_results;
                        let parsedRx = r.prescriptions;
                        let parsedVitals = r.vitals;
                        if (typeof parsedLab === 'string') try { parsedLab = JSON.parse(parsedLab); } catch (e) { }
                        if (typeof parsedRx === 'string') try { parsedRx = JSON.parse(parsedRx); } catch (e) { }
                        if (typeof parsedVitals === 'string') try { parsedVitals = JSON.parse(parsedVitals); } catch (e) { }
                        return { ...r, lab_results: parsedLab, prescriptions: parsedRx, vitals: parsedVitals };
                    });
                }
    ...
    const handleDownload = () => {
        ...
        try {
            ...
            // Download
            doc.save(`MetQ_Passport_${(profile.full_name || user?.full_name)?.replace(/\s+/g, '_') || 'Patient'}.pdf`);
        } catch (err) {
    ...
    return (
        <div className="min-h-screen pt-32 pb-20 px-8 bg-[#f8fafc]">
            ...
            {selectedRecord && (
                ...
                >
                    <div
                        className="w-full max-w-lg bg-white rounded-[2.5rem] premium-shadow overflow-hidden flex flex-col max-h-[90vh]"
                    ...
                    >
                        ...
                        <div className="p-8 space-y-6 overflow-y-auto flex-1">
                            ...
                            {/* Lab Results */}
                            {selectedRecord.lab_results && selectedRecord.lab_results.notes && (
                                <div>
                                    <div className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Lab Results</div>
                                    <p className="text-gray-600 font-medium text-sm">{selectedRecord.lab_results.notes}</p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="md:col-span-2 space-y-6">
                        ...
                        {filteredRecords.length > 0 ? filteredRecords.map((record, i) => (
                            <div key={i} className="bg-white p-8 rounded-[2.5rem] premium-shadow border border-gray-100 hover:border-medical-accent transition-all group cursor-pointer">
                                ...
                                {/* Lab Results Snippet */}
                                {record.lab_results && record.lab_results.notes && (
                                    <div className="mb-4 p-3 bg-gray-50 rounded-xl border border-gray-100">
                                        <div className="flex items-center gap-2 mb-1">
                                            <Activity size={12} className="text-medical-primary" />
                                            <span className="text-[10px] font-bold uppercase tracking-widest text-medical-primary">Lab Results</span>
                                        </div>
                                        <p className="text-xs text-gray-600 font-medium line-clamp-2">{record.lab_results.notes}</p>
                                    </div>
                                )}
